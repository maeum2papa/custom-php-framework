...
use \common\Cryption as Cryption;
class Library{
    ...
    function __construct(){
        ...
        if(!$_SESSION['sess'] && $_COOKIE['savelogin']){

            $this->cryption = new Cryption;
            $this->cryption->set(0,$this->cfg['cryption_key']);

            $cookieData = explode('^',$this->cryption->decrypt($_COOKIE['savelogin']));

            $query = "select idx, uid, name from ".$db->tables['doctor']." where idx='".$cookieData[0]."' and ip='".$cookieData[1]."'";
    ...
    function isSafari(){
        if (preg_match('/Safari/i', $_SERVER['HTTP_USER_AGENT']) && !preg_match('/Chrome/i', $_SERVER['HTTP_USER_AGENT'])){
            $this->cfg['safari'] = true;
        }else{
            $this->cfg['safari'] = false;
        }
    }
